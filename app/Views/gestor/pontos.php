...
$sql = "
    SELECT p.id, p.numero, p.logradouro, p.descricao, p.cidade, p.regiao,
           COALESCE(NULLIF(TRIM(cl.razao_social),''), NULLIF(TRIM(c.cliente),'')) AS cliente,
...
$campanhas  = $pdo->query("
    SELECT DISTINCT COALESCE(NULLIF(TRIM(c.nome),''), NULLIF(TRIM(c.campanha),'')) AS campanha
    FROM pontos p
    LEFT JOIN campanhas c ON c.ponto_id = p.id AND c.ativo = 1
    WHERE (p.ativo = 1 OR p.ativo IS NULL)
...
function gerarRelatorioExclusivos() {
    ...
    var html = '<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8">'
        + '<title>Relatório de Painéis Exclusivos · Impakto</title><style>'
        ...
        + '<p class="sub">Impakto Mídia OOH · gerado em ' + new Date().toLocaleString('pt-BR') + ' · ' + exclusivos.length + ' painel(is)</p>'
