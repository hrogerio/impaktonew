...
// Busca fotos
$stmt2 = $pdo->prepare("SELECT cf.*, p.numero, p.logradouro, p.bairro, p.cidade, p.regiao, p.latitude, p.longitude FROM checking_fotos cf JOIN pontos p ON p.id = cf.ponto_id WHERE cf.cliente=? AND cf.campanha=? AND cf.situacao=? AND cf.inicio<=>? AND cf.fim<=>? AND cf.ponto_id IN ($ph) ORDER BY p.numero ASC, cf.ordem ASC, cf.id ASC");
$stmt2->execute(array_merge([$cliente, $campanha, $situacao, $inicio, $fim], $pontoIds));
$todasFotos = $stmt2->fetchAll(PDO::FETCH_ASSOC);

if (empty($todasFotos)) { ob_end_clean(); http_response_code(400); die('Nenhuma foto encontrada.'); }
...
foreach ($campos as [$lbl, $val]) {
    $pdf->SetFont(FONT_MAIN, 'B', 9);    // ↑ era 7.5
    $pdf->SetTextColor(...$MUTED);
    $pdf->SetXY($RX, $y);
    $pdf->Cell(30, 6, s($lbl), 0, 0, 'L');   // ↑ era W=26, H=5

    $pdf->SetFont(FONT_MAIN, '', 10.5);   // ↑ era 8.5
    $pdf->SetTextColor(...$PRETO);
    $pdf->SetXY($RX + 30, $y);
    $pdf->Cell($RW - 30, 6, s($val), 0, 1, 'L');
    $y += 10;                             // ↑ era 8
}
...
if ($y < $yMax - 15) {
    // Cabeçalho "Pontos"
    $pdf->SetFont(FONT_MAIN, 'B', 8.5);   // ↑ era 7
    $pdf->SetTextColor(...$MUTED);
    $pdf->SetXY($RX, $y);
    ...
    foreach ($pontosExibidos as $i => $p) {
        // Coluna: primeiros $maxPorColuna na esquerda, o resto na direita
...
// ─────────────────────────────────────────────────────────────────────────────
$FH     = 30;           // ↑ era 28 — altura do rodapé (mm)
$FOTO_H = $PH - $FH;   // área da foto = 180mm

foreach ($todasFotos as $foto) {
    $pdf->AddPage();

    $imgPath = __DIR__ . '/../../../../' . $foto['caminho'];
    ...
    if (file_exists($imgPath)) {
        ...
        $pdf->Cell($PW, 9, s('Foto nao disponivel'), 0, 0, 'C');
    }
    ...
    $pdf->SetFont(FONT_MAIN, 'B', 14);   // ↑ era 12
    $pdf->SetTextColor(...$VERM);
    $pdf->SetXY(7, $FY + 8);            // ↑ era (6, FY+7)
    $pdf->Cell(24, 8, '#' . str_pad($foto['numero'], 3, '0', STR_PAD_LEFT), 0, 0, 'L');

    // Logradouro — PRETO
    $pdf->SetFont(FONT_MAIN, 'B', 11);   // ↑ era 9
    $pdf->SetTextColor(...$PRETO);
    $pdf->SetXY(33, $FY + 7);           // ↑ era (30, FY+6)
    $pdf->Cell(108, 6, s($foto['logradouro'] ?? ''), 0, 1, 'L');

    // Bairro · Cidade — cinza escuro
    $loc = implode(' - ', array_filter([$foto['bairro'] ?? '', $foto['cidade'] ?? '']));
    $pdf->SetFont(FONT_MAIN, '', 9.5);   // ↑ era 7.5
    $pdf->SetTextColor(80, 80, 80);
    $pdf->SetXY(33, $FY + 15);          // ↑ era (30, FY+12)
    ...
    if (!empty($foto['latitude']) && !empty($foto['longitude'])) {
        ...
        // Mede a largura real: padding + seta + gap + texto + padding
        $padL = 4; $arrowW = 7; $gap = 2; $padR = 5;
        ...
        // Borda fina cinza-azulada
        $pdf->SetDrawColor(180, 185, 200);
        $pdf->SetLineWidth(0.35);
        $pdf->Rect($btnX, $btnY, $btnW, $btnH);

        // Seta ↗ em vermelho Impakto
        $pdf->SetFont(FONT_MAIN, 'B', 9);
        $pdf->SetTextColor(...$VERM);
        $pdf->SetXY($btnX + $padL, $btnY + 1.6);
        $pdf->Cell($arrowW, 6, s('↗'), 0, 0, 'L');

        // "Ver no Google Maps" em cinza-escuro clicável
        $pdf->SetFont(FONT_MAIN, 'B', 9.5);
        $pdf->SetTextColor(40, 45, 60);
        $pdf->SetXY($btnX + $padL + $arrowW + $gap, $btnY + 1.6);
        $pdf->Cell($txtW + $padR, 6, s('Ver no Google Maps'), 0, 0, 'L', false, $mapsUrl);

        // Área inteira clicável
        $pdf->Link($btnX, $btnY, $btnW, $btnH, $mapsUrl);

        // Coordenadas em preto abaixo
    ...
    if (file_exists($logoRodape)) {
        $pdf->Image($logoRodape, $PW - 35, $FY + 7, 30);  // ↑ era w=29
    }
}
